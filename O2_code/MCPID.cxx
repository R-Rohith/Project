...
//	Filter col = aod::evsel::sel8 == true;
	...
	void init(InitContext const&)
	{
		...
		histos.add("cntp","counterpion",kTH1D,{{20,0,20,"category"}});
	}
	void process(soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator const& filteredCollisions, soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection,aod::pidBayesEl,aod::pidBayesPi,aod::pidBayesKa,aod::pidBayesPr,aod::pidTPCEl,aod::pidTPCPi,aod::pidTOFPi,aod::pidTOFbeta,aod::TracksExtra,aod::McTrackLabels>> const& tracks,aod::McParticles const&)
	{
		float tpccut,tofcut;
		for(auto track1: tracks)
		{
			if(!track1.has_mcParticle()){ histos.fill(HIST("cnt"),19.5);continue;}
			histos.fill(HIST("cntp"),0.5);
			if(fabs(track1.mcParticle().pdgCode())==11)histos.fill(HIST("cntp"),1.5);
			if(fabs(track1.mcParticle().pdgCode())==211)histos.fill(HIST("cntp"),2.5);
			if(fabs(track1.mcParticle().pdgCode())==321)histos.fill(HIST("cntp"),3.5);
			if(fabs(track1.mcParticle().pdgCode())==2212)histos.fill(HIST("cntp"),4.5);
			...
			histos.fill(HIST("nsigmatpc"),track1.p(),track1.tpcNSigmaPi());
			histos.fill(HIST("nsigmatof"),track1.p(),track1.tofNSigmaPi());
			...
			histos.fill(HIST("nsigmatpc1"),track1.p(),track1.tpcNSigmaPi());
			histos.fill(HIST("nsigmatof1"),track1.p(),track1.tofNSigmaPi());
			histos.fill(HIST("eta1"),track1.eta());
			histos.fill(HIST("phi1"),track1.phi());
			histos.fill(HIST("p1"),track1.p());
			histos.fill(HIST("cntp"),6.5);
			if(fabs(track1.mcParticle().pdgCode())==11)histos.fill(HIST("cntp"),7.5);
			if(fabs(track1.mcParticle().pdgCode())==211)histos.fill(HIST("cntp"),8.5);
			if(fabs(track1.mcParticle().pdgCode())==321)histos.fill(HIST("cntp"),9.5);
			if(fabs(track1.mcParticle().pdgCode())==2212)histos.fill(HIST("cntp"),10.5);
		}
		float tmp;
		int pid;
		for(auto track1: tracks)
		{
			...
			histos.fill(HIST("cntp"),12.5);
			if(fabs(track1.mcParticle().pdgCode())==11)histos.fill(HIST("cntp"),13.5);
			if(fabs(track1.mcParticle().pdgCode())==211)histos.fill(HIST("cntp"),14.5);
			if(fabs(track1.mcParticle().pdgCode())==321)histos.fill(HIST("cntp"),15.5);
			if(fabs(track1.mcParticle().pdgCode())==2212)histos.fill(HIST("cntp"),16.5);
		}
	}
};
...
//	Filter col = aod::evsel::sel8 == true;
	...
	void init(InitContext const&)
	{
		...
		histos.add("pdist","p distribution",kTH1D,{{100,0,3,"p"}});
		histos.add("pdist1","p distribution",kTH1D,{{100,0,3,"p"}});
		histos.add("cntk","counterkaon",kTH1D,{{20,0,20,"category"}});
	}
	void process(soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator const& filteredCollisions, soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection,aod::pidBayesEl,aod::pidBayesPi,aod::pidBayesKa,aod::pidBayesPr,aod::pidTPCEl,aod::pidTPCKa,aod::pidTOFKa,aod::pidTOFbeta,aod::TracksExtra,aod::McTrackLabels>> const& tracks,aod::McParticles const&)
	{
		...
		{
			if(!track1.has_mcParticle()){ histos.fill(HIST("cnt"),19.5);continue;}
			histos.fill(HIST("cntk"),0.5);
			if(fabs(track1.mcParticle().pdgCode())==11)histos.fill(HIST("cntk"),1.5);
			if(fabs(track1.mcParticle().pdgCode())==211)histos.fill(HIST("cntk"),2.5);
			if(fabs(track1.mcParticle().pdgCode())==321)histos.fill(HIST("cntk"),3.5);
			if(fabs(track1.mcParticle().pdgCode())==2212)histos.fill(HIST("cntk"),4.5);
			histos.fill(HIST("eta"),track1.eta());
			histos.fill(HIST("phi"),track1.phi());
			histos.fill(HIST("p"),track1.p());
			histos.fill(HIST("tpc"),track1.p(),track1.tpcSignal());
			...
			histos.fill(HIST("nsigmatof"),track1.p(),track1.tofNSigmaKa());
			tpccut = 2;
			tofcut = 2;
			if (track1.p() < 0.6) {
				if (track1.p() < 0.45)
					tpccut = 2;
				else if (track1.p() < 0.55)
					tpccut = 1;
				else
					tpccut = 0.6;
			...
			histos.fill(HIST("phi1"),track1.phi());
			histos.fill(HIST("p1"),track1.p());
			histos.fill(HIST("cntk"),6.5);
			if(fabs(track1.mcParticle().pdgCode())==11)histos.fill(HIST("cntk"),7.5);
			if(fabs(track1.mcParticle().pdgCode())==211)histos.fill(HIST("cntk"),8.5);
			if(fabs(track1.mcParticle().pdgCode())==321)histos.fill(HIST("cntk"),9.5);
			if(fabs(track1.mcParticle().pdgCode())==2212)histos.fill(HIST("cntk"),10.5);
		}
		float tmp;
		int pid;
		for(auto track1: tracks)
		{
			...
			histos.fill(HIST("phi2"),track1.phi());
			histos.fill(HIST("p2"),track1.p());
			histos.fill(HIST("cntk"),12.5);
			if(fabs(track1.mcParticle().pdgCode())==11)histos.fill(HIST("cntk"),13.5);
			if(fabs(track1.mcParticle().pdgCode())==211)histos.fill(HIST("cntk"),14.5);
			if(fabs(track1.mcParticle().pdgCode())==321)histos.fill(HIST("cntk"),15.5);
			if(fabs(track1.mcParticle().pdgCode())==2212)histos.fill(HIST("cntk"),16.5);
		}
	}
};
...
//	Filter col = aod::evsel::sel8 == true;
	...
	void init(InitContext const&)
	{
		...
		histos.add("pdist","p distribution",kTH1D,{{100,0,3,"p"}});
		histos.add("pdist1","p distribution",kTH1D,{{100,0,3,"p"}});
		histos.add("cnt","counterproton",kTH1D,{{20,0,20,"category"}});
	}
	void process(soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator const& filteredCollisions, soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection,aod::pidBayesEl,aod::pidBayesPi,aod::pidBayesKa,aod::pidBayesPr,aod::pidTPCEl,aod::pidTPCPr,aod::pidTOFPr,aod::pidTOFbeta,aod::TracksExtra,aod::McTrackLabels>> const& tracks,aod::McParticles const&)
	{
		float tpccut,tofcut;
		for(auto track1: tracks)
		{
			...
			histos.fill(HIST("tpc"),track1.p(),track1.tpcSignal());
			histos.fill(HIST("tof"),track1.p(),track1.beta());
			histos.fill(HIST("nsigmatpc"),track1.p(),track1.tpcNSigmaPr());
			histos.fill(HIST("nsigmatof"),track1.p(),track1.tofNSigmaPr());
			tpccut = 2.2;
			tofcut = 2;
			if (track1.p() < 1.1) {
				if (track1.p() < 0.85)
			...
			histos.fill(HIST("tpc1"),track1.p(),track1.tpcSignal());
			histos.fill(HIST("tof1"),track1.p(),track1.beta());
			histos.fill(HIST("nsigmatpc1"),track1.p(),track1.tpcNSigmaPr());
			histos.fill(HIST("nsigmatof1"),track1.p(),track1.tofNSigmaPr());
			histos.fill(HIST("eta1"),track1.eta());
			histos.fill(HIST("phi1"),track1.phi());
			histos.fill(HIST("p1"),track1.p());
...
{
	return {
	adaptAnalysisTask<Pion>(cfgc),
	adaptAnalysisTask<Kaon>(cfgc),
	adaptAnalysisTask<Proton>(cfgc),
	};
}
