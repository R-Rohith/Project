...
//		histos.add("rho2","rho2",kTH2D,{{864,0,864,"eta1phi1"},{864,0,864,"eta2phi2"}});
		histos.add("multPi","multPi",kTH1I,{{200,0,200,"multiplicity"}});
...
//		histos.add("phi","phi",kTH1D,{{100,-1,2*PI+1,"phi"}});
	...
	void process(soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator const& filteredCollisions, soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection,aod::pidTPCFullPi,aod::pidTOFPi,aod::pidTPCFullEl,aod::pidTOFEl,aod::pidTOFbeta,aod::TracksExtra>> const& tracks)
	{
		float tpccut,tofcut;
...
//				else tofcut=3;	
//				if(fabs(track1.tofNSigmaPi())>=tofcut) histos.fill(HIST("multPi"),2);//continue;		//tracks rejected by low pt tof
				}
...
//			}
			if(track1.pt()>0.6)			
			{
				histos.fill(HIST("multPi"),5);
				if(track1.hasTOF())
				{
				tofcut=3;
				if(fabs(track1.tofNSigmaPi())>=tofcut) histos.fill(HIST("multPi"),6);
				}
				else 	histos.fill(HIST("multPi"),7);			
...
//----------------------------------------------------------------------------------*/
//Electron---------------------------------------------------------------------
			if(fabs(track1.tpcNSigmaEl())<1.2) histos.fill(HIST("multPi"),3);//continue;			//Tracks rejected as el
//----------------------------------------------------------*/			
			histos.fill(HIST("multPi"),1);					//total Tracks 
...
//		histos.add("rho1","rho1",kTH2D,{{24,-0.8,0.8,"eta"},{36,0,2*PI,"phi"}});
//		histos.add("rho2","rho2",kTH2D,{{864,0,864,"eta1phi1"},{864,0,864,"eta2phi2"}});
		histos.add("multKa","multKa",kTH1I,{{200,0,200,"multiplicity"}});
//		histos.add("eta","eta",kTH1D,{{100,-1,1,"eta"}});
//		histos.add("phi","phi",kTH1D,{{100,-1,2*PI+1,"phi"}});
		histos.add("pt1","pt",kTH1D,{{100,0,5,"pt"}});
		histos.add("pt2","pt",kTH1D,{{100,0,5,"pt"}});
	...
	void process(soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator const& filteredCollisions, soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection,aod::pidTPCFullKa,aod::pidTOFKa,aod::pidTPCFullEl,aod::pidTOFEl,aod::pidTOFbeta,aod::TracksExtra>> const& tracks)
	{
		float tpccut,tofcut;
		for(auto track1:tracks)
		{
...
//				if(track1.pt()<1.5)tofcut=3;
//				else tofcut=3;	
				if(fabs(track1.tofNSigmaPi())>=tofcut) histos.fill(HIST("mult"),2);//continue;		//tracks rejected by low pt tof
				}
				else if(track1.pt()>=0.7)
					continue;
//			}
...
///for Kaon there's a small region with e, since its in low momentum TOF is not very effective.
			...
			if(track1.pt()<0.7)
			{
				...
				{
					tpccut=2;
//					if((fabs(track1.tofNSigmaKa())>tofcut)||(fabs(track1.tpcNSigmaKa())>tpccut))histos.fill(HIST("multKa"),2);//continue;
				}
				else
					if(fabs(track1.tpcNSigmaKa())>tpccut) continue;
			}
...
//----------------------------------------------------------------------------------*/
//Electron---------------------------------------------------------------------
			if(fabs(track1.tpcNSigmaEl())<1.2) histos.fill(HIST("multKa"),3);//continue;			//Tracks rejected as el
//----------------------------------------------------------*/			
			histos.fill(HIST("multKa"),1);					//total Tracks 
			histos.fill(HIST("bb4"),track1.pt(),track1.beta());
//			histos.fill(HIST("bb1_a"),track1.pt(),track1.tpcNSigmaEl());
			histos.fill(HIST("bb3"),track1.pt(),track1.tpcSignal());
//			histos.fill(HIST("proj1"),track1.tpcNSigmaEl());
...
//		histos.add("rho1","rho1",kTH2D,{{24,-0.8,0.8,"eta"},{36,0,2*PI,"phi"}});
//		histos.add("rho2","rho2",kTH2D,{{864,0,864,"eta1phi1"},{864,0,864,"eta2phi2"}});
		histos.add("multPr","multPr",kTH1I,{{200,0,200,"multiplicity"}});
//		histos.add("eta","eta",kTH1D,{{100,-1,1,"eta"}});
//		histos.add("phi","phi",kTH1D,{{100,-1,2*PI+1,"phi"}});
		histos.add("pt1","pt",kTH1D,{{100,0,5,"pt"}});
		histos.add("pt2","pt",kTH1D,{{100,0,5,"pt"}});
	...
	void process(soa::Filtered<soa::Join<aod::Collisions, aod::EvSels>>::iterator const& filteredCollisions, soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection,aod::pidTPCFullPr,aod::pidTOFPr,aod::pidTPCFullEl,aod::pidTOFEl,aod::pidTOFbeta,aod::TracksExtra>> const& tracks)
	{
		float tpccut,tofcut;
		for(auto track1:tracks)
		{
...
///Proton----------------------------------------------------------------------------little loss
			...
			if(track1.pt()<1.25)
			{
				...
				{
					tpccut=2.2;
//					if((fabs(track1.tofNSigmaPr())>tofcut)||(fabs(track1.tpcNSigmaPr())>tpccut))histos.fill(HIST("multPr"),2);//continue;
				}
				else
					if(fabs(track1.tpcNSigmaPr())>tpccut) continue;
			}
...
//----------------------------------------------------------------------------------*/
//Electron---------------------------------------------------------------------
			if(fabs(track1.tpcNSigmaEl())<1.2) histos.fill(HIST("multPr"),3);//continue;			//Tracks rejected as el
//----------------------------------------------------------*/			
			histos.fill(HIST("multPr"),1);					//total Tracks 
			histos.fill(HIST("bb4"),track1.pt(),track1.beta());
//			histos.fill(HIST("bb1_a"),track1.pt(),track1.tpcNSigmaEl());
			histos.fill(HIST("bb3"),track1.pt(),track1.tpcSignal());
//			histos.fill(HIST("proj1"),track1.tpcNSigmaEl());
...
{
	return {
	adaptAnalysisTask<pid1>(cfgc),
	adaptAnalysisTask<pid2>(cfgc),
	adaptAnalysisTask<pid3>(cfgc),
	};
}
/*
